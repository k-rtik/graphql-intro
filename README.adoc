...
The `verifyArtist()` and `verifyAlbums()` helper methods test the equality of the expected `Artist` and `Album` objects
and the objects returned by the GraphQL API.
The test methods, annotated with `@Test`, use the GraphQL client to test a query/mutation in the GraphQL API.
The `setupClientAndArtists()` method resets the `ArtistService` service, gets the expected `Artist` and `Album` objects
and builds the GraphQL client. This method is executed once before running the tests.

The `testGetArtist()` test method executes the `artist` query on a single artist and verifies that the correct object
is returned.

The `testGetUnknownArtist()` test method executes the `artist` query on a nonexistent artist and verifies that a
`GraphQLException` is triggered.

The `testGetArtists()` test method executes the `artists` query and verifies that all the returned objects are correct.

The `testGetArtistWithAlbumCount()` test method executes the `artist` query. The query is requested with the `albumCount`
field by using the `getArtistWithAlbumCount()`  method in the client which returns an `ArtistWithAlbumCount` object.
This test method verifies that the correct object is returned and that the object has the correct album count.

The `testAddArtistMutation()` test method executes the `addArtist` mutation to add a new artist to the service.
...
The `testResetMutation()` test method executes the `reset` mutation and verifies that the number of deleted artists
from the service is 1, since we added one new artist to the service.
